...
contract SupplyChain is Ownable, RetailerRole, ConsumerRole, FarmerRole, DistributorRole {
  ...
  function processItem(uint _upc) public verifyCaller(items[_upc].originFarmerID) harvested(_upc) onlyFarmer
  ...
  function packItem(uint _upc) public verifyCaller(items[_upc].originFarmerID) processed(_upc) onlyFarmer
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function sellItem(uint _upc, uint _price) public verifyCaller(items[_upc].originFarmerID) packed(_upc) onlyFarmer
  // Call modifier to check if upc has passed previous supply chain stage 
  ...
  function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyDistributor
    ...
    item.originFarmerID.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
  ...
  function shipItem(uint _upc) public verifyCaller(items[_upc].distributorID) sold(_upc) onlyDistributor
    // Call modifier to check if upc has passed previous supply chain stage
